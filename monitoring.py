...
def log_activity(user_id: int, role: str, action: str, detail: str = "") -> None:
    ...
    if detail:
        entry["detail"] = detail
        entry.update(_extract_structured_fields(detail))

    _append_structured("activity", entry)
...
def log_system_error(message: str, exc: Optional[BaseException] = None) -> None:
    ...
    _append_structured("system", entry)
...
def _append_structured(log_type: str, payload: Mapping[str, Any]) -> None:
    entry = dict(payload)
...
_DETAIL_KEY_PATTERN = re.compile(r"(?:^|[\s;,])([A-Za-z0-9_.-]+)=")


def _extract_structured_fields(detail: str) -> Dict[str, str]:
    fields: Dict[str, str] = {}
    idx = 0
    while True:
        match = _DETAIL_KEY_PATTERN.search(detail, idx)
        if not match:
            break
        key = match.group(1)
        value_start = match.end()
        next_match = _DETAIL_KEY_PATTERN.search(detail, value_start)
        value_end = next_match.start() if next_match else len(detail)
        value = detail[value_start:value_end].strip(" ;,\t")
        if key:
            fields[key] = value
        idx = value_end
    return fields
    ...
    *,
    ...
    for entry in _iter_structured_entries(path):
        ...
        if user_id is not None and str(entry.get("user_id")) != str(user_id):
            continue
        entry_command = entry.get("command") or entry.get("action")
        if command and str(entry_command) != command:
            continue
